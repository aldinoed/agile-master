...
app.get('/api/data', async (req, res) => {
      try {
            const sql = "SELECT p.id_perusahaan, p.nama_perusahaan, p.logo_perusahaan, p.profil_perusahaan, COUNT(m.siswa_id) AS jumlah_siswa, po.id_posisi, po.nama_posisi, GROUP_CONCAT(DISTINCT s.prodi) AS prodi, s.semester FROM perusahaan p LEFT JOIN posisi po ON p.id_perusahaan = po.perusahaan_id INNER JOIN magang m ON po.id_posisi = m.posisi_id LEFT JOIN siswa s ON m.siswa_id = s.id_siswa GROUP BY p.id_perusahaan, po.nama_posisi;"
            const hasilQuery = await executeQuery(sql);

            const companiesData = {};

            hasilQuery.forEach(item => {
                  const { id_perusahaan, nama_perusahaan, logo_perusahaan, profil_perusahaan, jumlah_siswa, id_posisi, nama_posisi, prodi, semester } = item;
...
app.get('/api/perusahaan/:id', async (req, res) => {
      try {
            ...
            const sql = `
            SELECT 
                  pr.id_perusahaan, pr.nama_perusahaan, pr.alamat, pr.kota, pr.provinsi, pr.profil_perusahaan, pr.logo_perusahaan,
                  p.id_posisi, p.nama_posisi,
                  s.id_siswa, s.nama_siswa, s.email, s.jenis_kelamin
            FROM 
                  perusahaan pr
            LEFT JOIN 
                  posisi p ON pr.id_perusahaan = p.perusahaan_id
            LEFT JOIN 
                  magang m ON p.id_posisi = m.posisi_id
            LEFT JOIN 
                  siswa s ON m.siswa_id = s.id_siswa
            WHERE 
                  pr.id_perusahaan = ${id_perusahaan};`;
            ...
            const formatData = (datas) => {
                  ...
                  return Object.values(hasil).map(item => ({
                        ...
                        logo_perusahaan: item.logo_perusahaan,
                        jumlah_siswa_total: item.jumlah_siswa_total,
                        posisi: Object.values(item.posisi).map(posisi => ({
                              ...posisi,
                              jumlah_siswa: posisi.jumlah_siswa || null,
                              siswa: posisi.siswa.length > 0 ? posisi.siswa : null
                        }))
                  }));
            };
...
app.post('/api/perusahaan/create', async (req, res) => {
      try {
            const { nama_perusahaan, profil_perusahaan, alamat, kota, provinsi, logo_perusahaan } = req.body;

            const sql = `INSERT INTO perusahaan (nama_perusahaan, profil_perusahaan, alamat, kota, provinsi, logo_perusahaan) VALUES (?,?,?,?,?,?);`;
...
app.put('/api/perusahaan/:id/delete', async (req, res) => {
      const id = req.params.id;
      try {
            const query = `UPDATE perusahaan SET is_disabled=1 WHERE id_perusahaan='${id}';`;
            const response = await executeQuery(query);
            console.log("🚀 ~ app.put ~ response:", response)
            if (response.affectedRows > 0) {
                  res.status(200).json({ status: true, message: "Berhasil hapus data perusahaan " });
            } else {
                  res.status(403).json({ status: false, message: "Gagal hapus data perusahaan " });
            }
      } catch (error) {
            res.status(500).send(error.message);
      }
